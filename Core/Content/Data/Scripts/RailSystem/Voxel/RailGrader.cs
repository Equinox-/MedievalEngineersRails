...
namespace Equinox76561198048419394.RailSystem.Voxel
{
    ...
    public static class RailGraderSystem
    {
        public static void Grade(Edge edge, bool fill, bool excavate)
        {
            ...
            MyVoxelCoordSystems.WorldPositionToVoxelCoord(planet.PositionLeftBottomCorner, ref minWorld,
                out minMutable);
            MyVoxelCoordSystems.WorldPositionToVoxelCoord(planet.PositionLeftBottomCorner, ref maxWorld,
                out maxMutable);
            ...
            for (var itr = new Vector3I_RangeIterator(ref minMutable, ref maxMutable); itr.IsValid(); itr.MoveNext())
            {
                ...
                var excavationDensity =
                    (byte) (MathHelper.Clamp(1 - (shapeEmpty?.GetDensity(ref wpo) ?? 0), 0, 1) * byte.MaxValue);
